...
function S3(uri, callback) {
    ...
    return TileJSON.call(this, uri, function(err, source) {
        ...
        // Allow S3 clients to be set in advance.
        if (source && source.bucket && S3.clients[source.bucket])
            source.client = S3.clients[source.bucket];
        callback(err, source);
    });
};
...
S3.prototype.startWriting = function(callback) {
    ...
    if (this.client || S3.clients[this.bucket]) {
        this.client = this.client || S3.clients[this.bucket];
    } else {
        ...
        this.client = S3.clients[this.bucket] = knox.createClient({

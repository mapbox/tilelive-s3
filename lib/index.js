...
function S3(uri, callback) {
    ...
    return TileJSON.call(this, uri, function(err, source) {
        ...
        // Create S3 client.
        // Attempts to read AWS_ACCESS_KEY_ID, AWS_SECRET_ACCESS_KEY,
        // AWS_SESSION_TOKEN from environment.
        var AWS_ACCESS_KEY_ID = process.env.AWS_ACCESS_KEY_ID || '';
        var AWS_SECRET_ACCESS_KEY = process.env.AWS_SECRET_ACCESS_KEY || '';
        var AWS_SESSION_TOKEN = process.env.AWS_SESSION_TOKEN || undefined;
        if (source && source.bucket && ((AWS_ACCESS_KEY_ID && AWS_SECRET_ACCESS_KEY) || uri.client)) {
            source.client = uri.client || knox.createClient({
                agent: TileJSON.agent,
                bucket: source.bucket,
                key: AWS_ACCESS_KEY_ID || '',
                secret: AWS_SECRET_ACCESS_KEY || '',
                token: AWS_SESSION_TOKEN || undefined,
                secure: false
            });
        }
...
S3.prototype._loadTile = function(z, x, y, callback) {
    TileJSON.prototype.getTile.call(this, z, x, y, function(err, data, headers) {
        if (err) return callback(err);

        // Backward compatibility for existing content on S3 with incorrect headers:
        // deflated pbfs but wrong Content-Type and missing Content-Encoding.
        var sniffedContentType = tiletype.contentType(data);
        if (sniffedContentType === 'application/x-protobuf' && headers['Content-Type'] !== 'application/x-protobuf') {

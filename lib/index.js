...
function S3(uri, callback) {
    ...
    return TileJSON.call(this, uri, function(err, source) {
        ...
        // Knox client can be provided by the caller.
        if (source && source.bucket && uri.client)
            source.client = uri.client;
...
S3.prototype.putGrid = function(z, x, y, data, callback) {
    ...
    var key = url.parse(this._prepareURL(this.data.grids[0], z, x, y)).pathname;
    var headers = {
...
S3.prototype.putTile = function(z, x, y, data, callback) {
    ...
    var key = url.parse(this._prepareURL(this.data.tiles[0], z, x, y)).pathname;
...
S3._retry = function retry(func, callback) {
    return function(err) {
        func._retry = func._retry || 0;
        func._retry++;
        if (func._retry >= 5) {
            callback(err);
        } else {
            setTimeout(function() { func() }, Math.pow(2, func._retry) * 1000);
        }
    };
};
...
S3.prototype.put = function(key, data, headers, callback) {
    ...
    function get() {
        ...
        s3.get(client.http(key), function(err, buffer, headers) {
            // Error.
            if (err && err.status !== 403 && err.status !== 404) return S3._retry(get, callback)(err);
    ...
    function put() {
        ...
        req.on('error', function(err) {
            if (semaphore) return;
            semaphore = true;
            S3._retry(put, callback)(err);
        });
        req.setTimeout(5e3, req.abort);
        req.on('response', function(res) {
            ...
            if (res.statusCode === 200) {
                ...
                res.on('end', function() {
                    var code = 'Unknown';
                    var matches = body.match(/<Code>(.+)<\/Code>/);
                    if (matches) code = matches[1];
                    if (semaphore) return;
                    semaphore = true;
                    return S3._retry(put, callback)(new Error('S3 put failed: ' + res.statusCode + ' ' + code));
                });
            }
        });
        req.end(data);
...
S3.prototype._lockingRefreshCreds = Locking(function(creds, callback) {
    creds.get(callback);
});

S3.prototype.refreshCreds = function(callback) {
    var source = this;

    // No creds means we've had knox client provided directly to the
    // constructor. Skip credential rotation.
    if (!source.creds) return callback();

    this._lockingRefreshCreds(source.creds, function(err) {
        if (err) return callback(err);
        source.client.key = source.client.options.key = source.creds.accessKeyId;
        source.client.secret = source.client.options.secret = source.creds.secretAccessKey;
        source.client.token = source.client.options.token = source.creds.sessionToken;
        callback();
    });
};
...
S3.prototype.startWriting = function(callback) {
    ...
    // Bail early if client was provided and set by contructor or if no bucket
    // was provided.
    if (source.client || !source.bucket) return callback && callback();

    // Get creds and create S3 client.
    var chain = new AWS.CredentialProviderChain();
    chain.resolve(function (err, creds) {
        if (err) return callback(new Error('Failed to find AWS credentials: ' + err));

        source.creds = creds;
        source.client = knox.createClient({

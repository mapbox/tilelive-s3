...
function S3(uri, callback) {
    ...
    // defaults
    // if true, force overwriting (don't check md5s before putting tiles)
    this._force = false;
     // if true, overwrite tiles that are the same if their expiration is passed
    this._checkExpiration = false;
    ...
    if (uri.protocol === 's3:' && uri.host) {
        ...
        this._force = query.hasOwnProperty('force') ? !!query.force : this._force;
        this._checkExpiration = query.hasOwnProperty('checkExpiration') ? !!query.checkExpiration : this._checkExpiration;
    ...
    if (this._force) console.log('forcing tile copy');
...
function managedError(err, context, grid) {
    ...
    if (!error || !error.retryable) console.error(err, error);
...
S3.prototype.put = function(key, data, headers, callback) {
    ...
    var checkExpiration = this._checkExpiration;
    ...
    (function get() {
        ...
        var req = s3.getObject(params, function(err, response) {
            ...
            // if current date is ahead of the expiration on S3,
            // update the tile without comparing the bodies
            if (checkExpiration && response.Expires) {
                var exp = new Date(response.Expires);
                if (exp < new Date) return put();
            }

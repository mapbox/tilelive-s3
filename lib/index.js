...
function S3(uri, callback) {
    ...
    return TileJSON.call(this, uri, function(err, source) {
        ...
        // Create S3 client and cache.
        if (source && source.bucket &&
            (S3.clients[source.bucket] ||
            (source.data.awsKey && source.data.awsSecret))) {
            source.client = S3.clients[source.bucket] || knox.createClient({
                agent: TileJSON.agent,
                key: source.data.awsKey,
                secret: source.data.awsSecret,
                bucket: source.bucket,
                token: source.data.awsToken,
                secure: false
            });
            S3.clients[source.bucket] = source.client;
        }
        callback(err, source);
    });
};
...
// Store/retrieve knox clients per bucket.
S3.clients = {};

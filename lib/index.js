...
function onRetry(res) {
    console.log('onRetry, res:', JSON.stringify(res));
...
S3.prototype.putTile = function(z, x, y, data, callback) {
    console.log('tilelive_s3.putTile start', z, x, y);
    ...
    console.log('tilelive_s3.putTile about to put', key);
    this.put(key, data, headers, function (err) {
        console.log('tilelive_s3.put err?', err);
        if (!err && source._eventsEnabled) source.emit('putTile', z, x, y, data.length);
        console.log('tilelive_s3.put callback(err)');
        return callback(err);
    });
};
...
S3.prototype.put = function(key, data, headers, callback) {
    console.log('tilelive_s3.put function start', key);
    ...
    (function get() {
        console.log('tilelive_s3.put.get start');
        var attempts = 5;
        stats.get++;
        console.log('tilelive_s3.put.get s3.getObject', JSON.stringify(params));
        var req = s3.getObject(params, function(err, response) {
            console.log('tilelive_s3.put.get s3.getObject err?', err, JSON.stringify(response));
            if (err) return put();
            ...
            console.log('tilelive_s3.put.get md5', oldmd5, newmd5);
            if (oldmd5 === newmd5) {
                stats.noop++;
                console.log('tilelive_s3.put.get md5 invoking callback');
                return callback();
            }

            console.log('tilelive_s3.put.get.put')
            put();
        }).on('retry', onRetry);
        console.log('tilelive_s3.put.get s3.getObject invoking req()');
        req();
    })();

    function put() {
        console.log('tilelive_s3.put.put start');
        ...
        console.log('tilelive_s3.put.put s3.putObject start', JSON.stringify(params));
        s3.putObject(params, function(err, response) {
            console.log('tilelive_s3 s3.putObject err?', err);
            if (err) return callback(managedError(err, this) || err);

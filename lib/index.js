...
S3.prototype.putTile = function(z, x, y, data, callback) {
    ...
    this.put(key, data, headers, function (err, meta) {
        if (!err && source._eventsEnabled) source.emit('putTile', z, x, y, data.length, meta && meta.deltaRatio);
        callback(err);
    });
};
...
S3.prototype.put = function(key, data, headers, callback) {
    ...
    (function get() {
        ...
        var req = s3.getObject(params, function(err, response) {
            ...
            // The ratio of new tile size to old tile size
            var deltaRatio = data.length / response.Body.length;

            if (deltaRatio <= 0.5) {
              stats.blocked++;
              params.Key += '.blocked';
              console.log(`WARNING: tile blocked @ s3://${params.Bucket}/${params.Key}`);
            }

            put({ deltaRatio: deltaRatio });
        }).on('retry', onRetry);
    })();
...
S3.prototype.stopWriting = function(callback) {
    ...
    console.log('renderStats: { '
      + `get: ${this._stats.get}, `
      + `put: ${this._stats.put}, `
      + `noop: ${this._stats.noop}, `
      + `blocked: ${this._stats.blocked}, `
      + `txin: ${this._stats.txin}, `
      + `txout: ${this._stats.txout} }`
    );

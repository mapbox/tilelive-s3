...
function S3(uri, callback) {
    ...
    function setup(err, data) {
        ...
        if (uri.data 
            && typeof process.env.AWSChinaMapId !== 'undefined' 
            && typeof uri.data.id !== 'undefined'
            &&  process.env.AWSChinaMapId == uri.data.id) {

            console.log('env mapid:' + process.env.AWSChinaMapId);
            console.log('uri mapid:' + uri.data.id);
            
            var s3Options= {
                accessKeyId: process.env.AWSChinaAccessToken,
                secretAccessKey: process.env.AWSChinaSecretKey,
                region: 'cn-north-1',
                maxRetries: 2,
                sslEnabled: true,
                httpOptions: {
                    connectTimeout: 100,
                    timeout: 10000 // 4min, default is 2min
                }
            };
            
            var tunnelingAgent = tunnel.httpsOverHttps({
                proxy: {
                    host: process.env.dxReverseProxy,
                    port: 443,
                    proxyAuth: process.env.dxProxyAuth,
                    headers: {
                    'User-Agent': 'Node'
                    }
                }
            });
            s3Options.httpOptions.agent = tunnelingAgent;
            source.client = uri.client || new AWS.S3(s3Options);
        }
        else
        {
